...
fn run_client(opts: & tpcoptions::TPCOptions, running: Arc<AtomicBool>) {
    ...
    let mut client = Client::new(client_id_str, running, sender, receiver, (opts.num_clients*opts.num_participants) as u64);
...
fn run_participant(opts: & tpcoptions::TPCOptions, running: Arc<AtomicBool>) {
    ...
    let (sender, receiver) = connect_to_coordinator(opts);
    let mut participant = Participant::new(participant_id_str, participant_log_path, running.clone(), opts.send_success_probability, opts.operation_success_probability, sender, receiver, opts.num_participants as u64);
